...
---
## This repo: 513sips-tools

- `Egady513/513sips-tools` auto-deploys to **www.513sips.com** on push to `main`
- Staging: `Egady513/513Sips-Staging`
- **No build step. Push equals deploy.** There is no CI to catch you.
- `gallery-data.json` is generated by GitHub Actions. Never edit it by hand.
- `nav.js` is the single source of truth for nav across all pages.

**Because push equals deploy, `main` is production.** Branch, review, then merge.
Claude Code sequence is `513sips-brand` then `513sips-qa` then `513sips-deploy`,
always in that order. Codex must hand off to Claude Code for the QA step.
